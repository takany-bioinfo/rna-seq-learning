...
このNotebookでは、**Arabidopsis thaliana** の **single-end RNA-Seq** 公開データを用いて、DEG解析（差次的遺伝子発現解析）に向けた前処理を行います。
...
※なお、このNotebookで生成した `count.txt` を用い、Rにて差次的遺伝子発現解析（DEG解析）を行ったR Markdownの出力（HTMLファイル）は以下よりご覧いただけます：

🔗 [DEG解析例を見る](https://takany-bioinfo.github.io/rna-seq-learning/deg_02.html)